...
- [x] hidden file や対象外ディレクトリの扱いを MVP 方針で決める
- [x] テキストファイルのみを扱う制約を UI と処理に反映する
...
- [x] ファイル拡張子に応じて Monaco の language を切り替える
- [ ] editor 変更を state に反映する
- [x] 読み込み失敗時のエラー表示を追加する
...
- [x] 選択ファイルの内容を読み込む API を用意する
- [x] 初回オープン時に editor へ内容を表示する
- [x] 同一ファイルの二重オープン防止を実装する
...
- [x] 保存 API を用意する
- [x] worktree 外パスへ書き込まないガードを入れる
- [x] 保存操作を UI から実行できるようにする
- [x] 保存成功時に未保存状態を解消する
- [x] 保存失敗時の復旧しやすいエラー表示を追加する
...
- Checklist
- [x] editor tab の状態管理を定義する
- [x] 複数ファイルをタブで開けるようにする
- [x] タブ切替で editor 内容が正しく切り替わるようにする
- [x] タブ close を実装する
- [x] editor 変更を state に反映する
- [x] 未保存状態をタブ表示に反映する
- Done
- 複数タブを扱える
- 未保存状態を UI 上で識別できる
