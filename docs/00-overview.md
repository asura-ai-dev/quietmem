...
- アプリ名: QuietMem
- 一言定義: 複数 AI agent を memory・editor・worktree・terminal・cron 込みで運用できるデスクトップ管理アプリ
...
- コード / 文書 / prompt / 設定 / 実行管理 / 手動ターミナル操作を 1 つの GUI にまとめる
...
- agent を並列実行できる
- 実行状況を可視化できる
- active worktree で terminal を開いて手動コマンドを実行できる
- current worktree を VS Code / Cursor / Terminal / Finder で開ける
- 複数 agent の並列実行状態を 1 画面で把握できる
...
- Execution: CLI adapter + integrated terminal
...
- [L11-Integrated-Terminal](./L11-integrated-terminal.md)
- [L12-Parallel-Agent-Orchestration](./L12-parallel-agent-orchestration.md)

## 9. Assumptions

- MVP の実行対象は Claude Code や Codex などのローカル CLI 型 agent を優先する
- 将来拡張は考慮するが、MVP はローカル CLI 実行を前提にし、GUI からの実行管理は CLI adapter と統合ターミナルで扱う
